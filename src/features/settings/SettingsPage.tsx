  ...
  return (
    <div className="max-w-xl">
      ...
      <section className="mt-6 rounded-lg border bg-background p-4">
        ...
        <p className="mt-1 text-sm text-muted-foreground">退出登录与密码修改。</p>

        <div className="mt-3">
          <button
            className="inline-flex items-center justify-center rounded-md border px-3 py-2 text-sm hover:bg-muted/60"
            onClick={() => auth.logout()}
          >
            退出登录
          </button>
      ...
      <section className="mt-4 rounded-lg border bg-background p-4">
        ...
        <div className="mt-4">
          ...
          {confirmPassword && newPassword !== confirmPassword ? (
            <div className="mt-2 text-sm text-destructive">两次输入的新密码不一致。</div>
          ) : null}
        </div>
        ...
        {mutation.isSuccess ? <div className="mt-3 text-sm text-green-600">修改成功。</div> : null}
